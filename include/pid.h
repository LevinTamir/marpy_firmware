...
//
//   PID pid;
//   pid_init(&pid, 50.0f, 10.0f, 0.0f, 50.0f);   // kp, ki, kd, i_term_max
//
...
// Update the gains in-place. Pass `reset_state = true` to clear accumulated
// history (recommended when the gain change is large, to avoid an output jolt).
...
// When all gains are 0, returns 0 (PID effectively disabled).
