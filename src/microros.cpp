...
// stamps, causing /tf chains rooted at /odom to render the wheels stuck
// at the world origin in RViz.
...
static void cmd_vel_cb(const void *msgin) {
  ...
  motors_set_velocity_from_twist((float)m->linear.x, (float)m->angular.z);
}
...
static void init_imu_msg() {
  ...
  // Orientation not provided (raw MPU6050 has none; let any future filter own it).
...
void microros_spin_some() {
  ...
  // Naive WiFi reconnect: if the link drops, kick the radio.
...
void microros_publish_joint_state(float left_pos_rad,  float right_pos_rad,
  ...
  joint_state_msg.position.data[0] = (double)left_pos_rad;
  joint_state_msg.position.data[1] = (double)right_pos_rad;
  joint_state_msg.velocity.data[0] = (double)left_vel_radps;
  joint_state_msg.velocity.data[1] = (double)right_vel_radps;
