...
float pid_step(PID *pid, float setpoint, float measured, uint32_t now_us) {
  ...
  // (e.g. micros() rolled over, or the loop stalled). Either way, the
  // derivative term would explode, so just return 0 and resync.
  if (dt <= 0.0f || dt > 0.5f) {
    pid->prev_error = setpoint - measured;
    return 0.0f;
  }
  ...
  // Accumulate error * dt, then clamp the resulting I-contribution (ki *
  // integral) to i_term_max. We clamp the integral itself (not the output)
